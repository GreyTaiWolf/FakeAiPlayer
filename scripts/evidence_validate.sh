...
EOF
  ...
  log_summary_count="$(grep -acE '\[FakeAiPlayer Verify\] summary' "$canonical/server.log" 2>/dev/null || true)"
  log_summary="$(grep -aE '\[FakeAiPlayer Verify\] summary' "$canonical/server.log" 2>/dev/null | tail -1 || true)"
...
run_self_test() (
  ...
  local -a generated
  fixture="$(mktemp "${TMPDIR:-/tmp}/fakeaiplayer-evidence-self-test.XXXXXX")"
  generated=()
  cleanup_self_test() {
    local path
    rm -f -- "$fixture" "${link:-}"
    ...
    rmdir "$(cd "${TMPDIR:-/tmp}" && pwd -P)/fakeaiplayer-evidence-runs" 2>/dev/null || true
  }
  trap cleanup_self_test EXIT

  printf '%s\n' '[Server thread/INFO]: Seed: [424242]' \
    '[Server thread/INFO]: [FakeAiPlayer Verify] summary 1/1 PASS {evidence_self_test=PASS}' > "$fixture"
