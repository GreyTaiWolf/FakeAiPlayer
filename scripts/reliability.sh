...
# 解决"real_* flaky、特定 chunk 才坏":先量化多坏/坏在哪阶段,再用 /fakeaiplayer snapshot 抓现场冻成确定性场景真修。
...
run_one() {
  ...
  summary=$(echo "$out" | grep -E "\[FakeAiPlayer Verify\] summary" | tail -1); summary="${summary#*summary }"
...
# 批量同代码多 seed:先 clean 编译通用源码、Fabric 源码和测试 harness，
# 之后各局复用 Gradle 的 up-to-date 输出(代码冻结,省 N-1 次全量重编)。
echo "[reliability] compiling once (clean) before batch ..."
./gradlew --stop >/dev/null 2>&1
if ./gradlew --no-daemon --rerun-tasks --no-build-cache clean \
    :common:classes :fabric:classes :fabric:gametestClasses >/dev/null 2>&1; then
  echo "[reliability] prebuilt OK, runs will reuse module outputs"
else
