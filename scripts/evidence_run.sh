...
LOADER_VERSION="$(harness_gradle_property fabric_loader_version)"
...
LOCK_PATH="$TMP_BASE/fakeaiplayer-evidence-$REPO_KEY.lock"
...
cleanup() {
  ...
  if [[ -n "$SERVER_RUN_DIR" && -d "$SERVER_RUN_DIR" && ! -L "$SERVER_RUN_DIR" ]]; then
    case "$SERVER_RUN_DIR" in
      "$EXEC_ROOT"/fabric/build/run/evidence-*) chmod -R u+w "$SERVER_RUN_DIR" 2>/dev/null || true; rm -rf -- "$SERVER_RUN_DIR" ;;
    esac
  fi
  if [[ -n "$RUN_DIR" && -d "$RUN_DIR" && ! -L "$RUN_DIR" ]]; then
    case "$RUN_DIR" in "$TMP_BASE"/fakeaiplayer-evidence-runs/run.*) rm -rf -- "$RUN_DIR" ;; esac
  fi
...
mkdir -p "$TMP_BASE/fakeaiplayer-evidence-runs" || exit 3
harness_assert_not_symlink "$TMP_BASE/fakeaiplayer-evidence-runs" 'temporary run root' || exit 3
RUN_DIR="$(mktemp -d "$TMP_BASE/fakeaiplayer-evidence-runs/run.${RUN_ID}.XXXXXX")" || exit 3
...
harness_assert_not_symlink "$EXEC_ROOT/fabric" fabric_project_directory || exit 3
harness_assert_not_symlink "$EXEC_ROOT/fabric/build" fabric_build_directory || exit 3
mkdir -p "$EXEC_ROOT/fabric/build" || exit 3
harness_assert_not_symlink "$EXEC_ROOT/fabric/build" fabric_build_directory || exit 3
harness_assert_not_symlink "$EXEC_ROOT/fabric/build/run" fabric_build_run_directory || exit 3
mkdir -p "$EXEC_ROOT/fabric/build/run" || exit 3
harness_assert_not_symlink "$EXEC_ROOT/fabric/build/run" fabric_build_run_directory || exit 3
SERVER_RUN_DIR="$EXEC_ROOT/fabric/$RELATIVE_SERVER_RUN_DIR"
...
printf 'eula=true\n' > "$SERVER_RUN_DIR/eula.txt" || exit 3
{
  ...
  printf 'motd=FakeAiPlayer isolated evidence run\n'
...
# through the child environment and is represented by a redaction marker below.
{
  ...
  printf '}\n'
} > "$SERVER_RUN_DIR/config/fakeaiplayer.json" || exit 3
...
if [[ -n "$FIXTURE_LOG" ]]; then
...
else
  {
    ...
    printf '%s\n' '  def fabricProject = gradle.rootProject.findProject(":fabric")'
    printf '%s\n' '  if (fabricProject != null) {'
    printf '%s\n' '    fabricProject.tasks.matching { it.name == "runHarnessServer" }.configureEach {'
    printf '%s\n' '      workingDir = new File(System.getenv("FAKEAIPLAYER_HARNESS_RUN_DIR"))'
    printf '%s\n' '      doFirst {'
    printf '%s\n' '        def expected = new File(System.getenv("FAKEAIPLAYER_HARNESS_RUN_DIR")).canonicalFile'
    printf '%s\n' '        def actual = getWorkingDir().canonicalFile'
    printf '%s\n' '        if (actual != expected) { throw new GradleException("FakeAiPlayer evidence workingDir mismatch") }'
    printf '%s\n' '        println("FAKEAIPLAYER_EVIDENCE_WORKDIR=" + actual)'
    printf '%s\n' '        println("FAKEAIPLAYER_EVIDENCE_JAVA_HOME=" + System.getProperty("java.home"))'
    printf '%s\n' '        println("FAKEAIPLAYER_EVIDENCE_JAVA_VERSION=" + System.getProperty("java.version"))'
    printf '%s\n' '        println("FAKEAIPLAYER_EVIDENCE_JAVA_VENDOR=" + System.getProperty("java.vendor"))'
    printf '%s\n' '      }'
    printf '%s\n' '    }'
    printf '%s\n' '  }'
    printf '%s\n' '}'
  ...
  if [[ $WITH_LLM -eq 1 ]]; then
    env AIBOT_PROFILE="$PROFILE" FAKEAIPLAYER_HARNESS_RUN_DIR="$SERVER_RUN_DIR" \
      "$EXEC_ROOT/gradlew" --no-daemon --console=plain --no-build-cache \
      -p "$EXEC_ROOT" -PfakeaiplayerHarnessRunDir="$RELATIVE_SERVER_RUN_DIR" \
      -I "$RUN_DIR/evidence.init.gradle" :fabric:runHarnessServer <&9 >> "$STAGING/server.log" 2>&1 &
  else
    env -u DEEPSEEK_API_KEY AIBOT_PROFILE="$PROFILE" FAKEAIPLAYER_HARNESS_RUN_DIR="$SERVER_RUN_DIR" \
      "$EXEC_ROOT/gradlew" --no-daemon --console=plain --no-build-cache \
      -p "$EXEC_ROOT" -PfakeaiplayerHarnessRunDir="$RELATIVE_SERVER_RUN_DIR" \
      -I "$RUN_DIR/evidence.init.gradle" :fabric:runHarnessServer <&9 >> "$STAGING/server.log" 2>&1 &
  fi
  ...
  if grep -aFq "FAKEAIPLAYER_EVIDENCE_WORKDIR=$SERVER_RUN_DIR" "$STAGING/server.log" 2>/dev/null; then
  ...
  if [[ "$READY" == yes ]]; then
    ...
    printf 'fakeaiplayer spawn EvidenceBot assistant\n' >&9 || READY=no
    sleep 5
    printf 'fakeaiplayer verify %s\n' "$SCENARIO" >&9 || READY=no
    for ((i = 0; i < MAXWAIT; i++)); do
      grep -aqE '\[FakeAiPlayer Verify\] summary' "$STAGING/server.log" 2>/dev/null && break
      kill -0 "$SERVER_PID" 2>/dev/null || break
      sleep 1
    done
  fi
...
JAVA_HOME_ACTUAL="$(sed -n 's/^FAKEAIPLAYER_EVIDENCE_JAVA_HOME=//p' "$STAGING/server.log" | tail -1)"
JAVA_VERSION_ACTUAL="$(sed -n 's/^FAKEAIPLAYER_EVIDENCE_JAVA_VERSION=//p' "$STAGING/server.log" | tail -1)"
JAVA_VENDOR_ACTUAL="$(sed -n 's/^FAKEAIPLAYER_EVIDENCE_JAVA_VENDOR=//p' "$STAGING/server.log" | tail -1)"
...
SUMMARY="$(grep -aE '\[FakeAiPlayer Verify\] summary' "$STAGING/server.log" 2>/dev/null | tail -1 || true)"
