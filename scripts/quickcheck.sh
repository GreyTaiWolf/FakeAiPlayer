#!/bin/bash
# 快速验证:排除一切 Gradle/Loom 缓存,确认 Fabric runServer 跑的是含 food 的最新 class。
...
sleep 1
rm -rf common/build fabric/build neoforge/build 2>/dev/null
rm -f fabric/run/server/world/fakeaiplayer/runtime.json \
  fabric/run/server/world/fakeaiplayer/bots.json 2>/dev/null
rm -f "$FIFO"; mkfifo "$FIFO"
sleep 100000 > "$FIFO" & HOLDER=$!

# 一条 invocation:clean + runServer,全量编译(无增量 stale)+ 启动
./gradlew --no-daemon --console=plain clean :fabric:runServer < "$FIFO" >> "$LOG" 2>&1 & SRV=$!
...
if [ "$READY" = 1 ]; then
  sleep 2
  echo "fakeaiplayer spawn QCBot assistant" > "$FIFO"; sleep 5
  echo "fakeaiplayer verify food" > "$FIFO"; sleep 15
fi

echo "[qc] ===== food 是否被识别 ====="
if grep -q "unknown feature" "$LOG" 2>/dev/null; then
  echo "[qc] RESULT = STILL_UNKNOWN (food 没进运行 class)"
elif grep -qE "goal_plan.*Food|FakeAiPlayer Verify.*food|step=.*猎|step=.*采集" "$LOG" 2>/dev/null; then
  echo "[qc] RESULT = FOOD_RECOGNIZED (verify food 开跑了!)"
else
  echo "[qc] RESULT = INCONCLUSIVE"
fi
grep -E "unknown feature|goal_plan|FakeAiPlayer Verify" "$LOG" 2>/dev/null | tail -4
