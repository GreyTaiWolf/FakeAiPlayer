...
FEATURE="${1:-food}"
MAXWAIT="${2:-900}"
SEED_VALUE="${SEED:-${FAKEAIPLAYER_TEST_SEED:-${AIBOT_TEST_SEED:-20260610}}}"
...
if [[ "$PROFILE_VALUE" == operator ]]; then
  args+=(--operator-capabilities "${FAKEAIPLAYER_OPERATOR_CAPABILITIES:-${AIBOT_OPERATOR_CAPABILITIES:-all}}")
fi
...
output="$(mktemp "${TMPDIR:-/tmp}/fakeaiplayer-foodtest.XXXXXX")"
