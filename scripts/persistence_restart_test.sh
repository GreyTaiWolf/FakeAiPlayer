...
SERVER_RUN_DIR="$ROOT/fabric/$RELATIVE_RUN_DIR"
REPORT_DIR="$ROOT/fabric/build/reports/persistence-restart/$STAMP"
PORT="$(harness_choose_port)"
TEMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/fakeaiplayer-restart.XXXXXX")"
FIFO=""
SERVER_PID=""
FD_OPEN=0
SUCCESS=0
...
} > "$SERVER_RUN_DIR/config/fakeaiplayer.json"
...
start_server() {
  ...
  exec 9<> "$FIFO"
  FD_OPEN=1
  env -u DEEPSEEK_API_KEY AIBOT_PROFILE=strict_survival FAKEAIPLAYER_TEST_PORT="$PORT" \
    "$ROOT/gradlew" --no-daemon --console=plain --no-build-cache \
    -p "$ROOT" -PfakeaiplayerHarnessRunDir="$RELATIVE_RUN_DIR" :fabric:runHarnessServer \
    <&9 >> "$log_file" 2>&1 &
  SERVER_PID=$!
  for ((i = 0; i < 480; i++)); do
...
start_server "$phase1"
printf 'fakeaiplayer spawn RestartBob assistant\n' >&9
printf 'fakeaiplayer harness restart-stage RestartBob\n' >&9
wait_for_outcome "$phase1" \
  '[FakeAiPlayer Harness] restart-stage STARTED' \
  '[FakeAiPlayer Harness] restart-stage FAIL' 120 || {
  printf 'persistence-restart: mission start failed; see %s\n' "$phase1" >&2
  exit 1
}
poll_command_until "$phase1" \
  'fakeaiplayer harness restart-stage-check RestartBob' \
  '[FakeAiPlayer Harness] restart-stage-check PASS' \
  '[FakeAiPlayer Harness] restart-stage-check FAIL' 120 || {
...
start_server "$phase2"
printf 'fakeaiplayer harness restart-check RestartBob\n' >&9
...
poll_command_until "$phase2" \
  'fakeaiplayer harness restart-progress RestartBob' \
  '[FakeAiPlayer Harness] restart-progress PASS' \
  '[FakeAiPlayer Harness] restart-progress FAIL' 180 || {
