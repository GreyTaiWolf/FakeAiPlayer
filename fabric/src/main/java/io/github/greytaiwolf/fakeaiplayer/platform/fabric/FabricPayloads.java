...
final class FabricPayloads {
    ...
    static void register() {
        ...
        PayloadTypeRegistry.playC2S().register(SetBotAiCredentialC2S.ID, SetBotAiCredentialC2S.CODEC);
        PayloadTypeRegistry.playC2S().register(RestoreBotAiCredentialC2S.ID, RestoreBotAiCredentialC2S.CODEC);
        ...
        PayloadTypeRegistry.playS2C().register(OpenBotAiSetupS2C.ID, OpenBotAiSetupS2C.CODEC);
        PayloadTypeRegistry.playS2C().register(BotAiCredentialStatusS2C.ID, BotAiCredentialStatusS2C.CODEC);
        ...
        ServerPlayNetworking.registerGlobalReceiver(SetBotAiCredentialC2S.ID, (payload, context) ->
                context.server().execute(() -> service.handleSetBotAiCredential(context.player(), payload)));
        ServerPlayNetworking.registerGlobalReceiver(RestoreBotAiCredentialC2S.ID, (payload, context) ->
                context.server().execute(() -> service.handleRestoreBotAiCredential(context.player(), payload)));
