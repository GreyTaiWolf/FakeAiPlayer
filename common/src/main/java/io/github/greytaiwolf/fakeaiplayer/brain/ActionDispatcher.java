    ...
    private ToolDefinition.ToolResult invoke(AIPlayerEntity bot, ChatToolCall call) {
        try {
            ...
            if (isManualMiningOrMove(call)
                    && io.github.greytaiwolf.fakeaiplayer.goal.GoalExecutor.INSTANCE.recentlyFailed(bot, GOAL_FAIL_GUARD_TICKS)) {
                BotLog.warn(io.github.greytaiwolf.fakeaiplayer.log.LogCategory.COMM, bot, "manual_mining_blocked", "tool", call.name());
            ...
            BotLog.warn(io.github.greytaiwolf.fakeaiplayer.log.LogCategory.COMM, bot, "tool_bad_arg", "tool", call.name(), "reason", reason);
