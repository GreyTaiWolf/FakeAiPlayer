...
public final class FarmTask extends AbstractTask {
    ...
    private void survey(AIPlayerEntity bot) {
        ...
        BlockPos.betweenClosedStream(areaCenter.offset(-radius, -1, -radius), areaCenter.offset(radius, 1, radius))
                ...
                .filter(pos -> io.github.greytaiwolf.fakeaiplayer.mode.ObservableWorldQuery.canObserveBlock(bot, pos)
                        || io.github.greytaiwolf.fakeaiplayer.mode.ObservableWorldQuery.canObserveBlock(bot, pos.above()))
    ...
    private void depositTransfer(AIPlayerEntity bot) {
        if (depositContainerPos == null
                ...
                || !io.github.greytaiwolf.fakeaiplayer.mode.ObservableWorldQuery.canObserveBlock(bot, depositContainerPos)) {
            phase = Phase.DEPOSIT;
            return;
        }
    ...
    private boolean hasImmatureCrops(AIPlayerEntity bot, ServerLevel world) {
        return BlockPos.betweenClosedStream(areaCenter.offset(-radius, -1, -radius), areaCenter.offset(radius, 1, radius))
                .filter(ground -> io.github.greytaiwolf.fakeaiplayer.mode.ObservableWorldQuery.canObserveBlock(bot, ground)
                        || io.github.greytaiwolf.fakeaiplayer.mode.ObservableWorldQuery.canObserveBlock(bot, ground.above()))
                .anyMatch(ground -> {
                    BlockPos cropPos = ground.above();
