...
public final class HuntTask extends AbstractTask {
    ...
    private static void surfaceIfUnderground(AIPlayerEntity bot) {
        ...
        int surfaceY = world.getHeight(net.minecraft.world.level.levelgen.Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
                feet.getX(), feet.getZ());
    ...
    protected void onTick(AIPlayerEntity bot) {
        EpisodeMemory.INSTANCE.recordTrail(bot.getUUID(), bot.blockPosition()); // 工作记忆:轨迹(roam 避重用)
    ...
    private BlockPos obstacleToward(AIPlayerEntity bot) {
        net.minecraft.core.Direction dir = bot.getDirection();
        ServerLevel world = bot.serverLevel();
        BlockPos ahead = bot.blockPosition().relative(dir);
        for (BlockPos p : new BlockPos[]{ahead, ahead.above()}) {
            net.minecraft.world.level.block.state.BlockState st = world.getBlockState(p);
