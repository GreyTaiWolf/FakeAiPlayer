...
public final class GuardTask extends AbstractTask {
    ...
    private BlockPos resolveGuardPoint(AIPlayerEntity bot) {
        if (!targetPlayerName.isBlank()) {
            ServerPlayer player = bot.getServer().getPlayerList().getPlayerByName(targetPlayerName);
            return player != null && player.serverLevel() == bot.serverLevel() ? player.blockPosition().immutable() : null;
        }
        if (fixedPoint != null) {
            return fixedPoint;
        }
        ...
        return owner.filter(player -> player.serverLevel() == bot.serverLevel())
