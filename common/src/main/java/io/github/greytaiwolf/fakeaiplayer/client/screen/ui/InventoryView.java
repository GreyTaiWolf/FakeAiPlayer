...
public final class InventoryView implements PanelComponent {
    ...
    public void render(GuiGraphics context, int mouseX, int mouseY, float delta, Font renderer) {
        ...
        context.drawString(renderer, Theme.tr("inventory.fakeaiplayer.section_equip"), gridX, y, Theme.TEXT_DIM);
        for (int i = 0; i < equipSlots.length; i++) {
            int gx = gridX + i * SLOT;
        ...
        // —— AI 背包 ——
        context.drawString(renderer, Theme.tr("inventory.fakeaiplayer.section_ai"), gridX, aiLabelY, Theme.TEXT_DIM);
        ...
        // —— 玩家背包 ——
        context.drawString(renderer, Theme.tr("inventory.fakeaiplayer.section_self"), gridX, plLabelY, Theme.TEXT_DIM);
        if (playerInv != null) {
        ...
        if (!hovered.isEmpty()) {
            context.renderTooltip(renderer, hovered, mouseX, mouseY);
        }
    }
    ...
    private static ItemStack stack(BotSnapshotS2C.ItemEntry entry) {
        Item item = BuiltInRegistries.ITEM.getOptional(ResourceLocation.parse(entry.itemId())).orElse(Items.BARRIER);
