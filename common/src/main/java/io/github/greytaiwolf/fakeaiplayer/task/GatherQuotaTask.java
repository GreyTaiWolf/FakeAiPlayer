...
public final class GatherQuotaTask extends AbstractTask {
    ...
    public String describe() {
        return "Gathering " + BuiltInRegistries.ITEM.getKey(targetItem) + " " + countSoFar + "/" + targetCount + " phase=" + phase;
    }
    ...
    private void survey(AIPlayerEntity bot) {
        ...
        if (exploreHops > 0 && exploredSinceFind) {
            io.github.greytaiwolf.fakeaiplayer.memory.EpisodeLog.INSTANCE.record(bot,
                    io.github.greytaiwolf.fakeaiplayer.memory.EpisodeLog.Type.RESOURCE_FOUND, targetPos,
                    BuiltInRegistries.BLOCK.getKey(bot.serverLevel().getBlockState(targetPos).getBlock()).toString());
    ...
    private void goToTarget(AIPlayerEntity bot) {
        ...
        if (bot.isInWater()) {
            ...
            EpisodeMemory.INSTANCE.exclude(bot.getUUID(), targetPos,
                    bot.getServer().getTickCount(), EpisodeMemory.TTL_UNREACHABLE);
