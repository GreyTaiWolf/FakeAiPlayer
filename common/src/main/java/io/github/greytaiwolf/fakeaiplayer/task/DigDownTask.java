...
public final class DigDownTask extends AbstractTask {
    ...
    protected void onStart(AIPlayerEntity bot) {
        ...
        if (!dryColumn(world, bot.blockPosition())) {
            ...
            for (int dist : new int[]{4, 8, 14}) {
                for (int[] d : dirs) {
                    ...
                    int ty = world.getHeight(net.minecraft.world.level.levelgen.Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, x, z);
                    ...
                    if (io.github.greytaiwolf.fakeaiplayer.pathfinding.Standability.isStandable(world, cand)
                        ...
                        boolean moved = io.github.greytaiwolf.fakeaiplayer.mode.CapabilityRuntime.run(
                                ...
                                "dig_down_dry_relocate", () -> {
                                    ...
                                    bot.teleportTo(world, x + 0.5D, ty, z + 0.5D,
        ...
        startPos = bot.blockPosition();   // 记井口,采够后回这里
        returning = false;
        returnStartTick = 0;
    }
    ...
    protected void onTick(AIPlayerEntity bot) {
        ...
        if (elapsed - lastProgressTick > NO_PROGRESS_LIMIT) {
            ...
            ServerLevel worldNow = bot.serverLevel();
    ...
    private boolean rotateStair(ServerLevel world, BlockPos feet) {
        for (int i = 0; i < HDIRS.length; i++) {
            ...
            BlockPos ahead = feet.relative(HDIRS[stairDirIndex]);
    ...
    private void digHorizontal(AIPlayerEntity bot, ServerLevel world, BlockPos feet) {
        for (int tries = 0; tries < HDIRS.length; tries++) {
            ...
            if (isLava(world, side) || isLava(world, side.above()) || isLava(world, side.below())
                    || isWater(world, side) || isWater(world, side.above()) || isWater(world, side.below())) {
            ...
            miner.cancel(bot);
            bot.getActionPack().startWalkTo(side.getCenter());
            return;
        }
