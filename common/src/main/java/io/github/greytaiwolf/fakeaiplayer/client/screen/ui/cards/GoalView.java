...
public final class GoalView extends PanelCard {
    ...
    protected void renderBody(GuiGraphics context, int mouseX, int mouseY, float delta, Font renderer, int bx, int by, int bw, int bh) {
        ...
        String title = snapshot.goalTitle().isBlank() ? Theme.tr("goal.fakeaiplayer.untitled") : snapshot.goalTitle();
        ...
        context.drawString(renderer, Theme.tr("goal.fakeaiplayer.progress", currentNumber, total), bx, by + 12, Theme.TEXT_DIM);
        ...
        for (int i = 0; i < maxRows && start + i < rows; i++) {
            ...
            context.drawString(renderer, trim(renderer, marker + " " + snapshot.goalSteps().get(idx), bw),
