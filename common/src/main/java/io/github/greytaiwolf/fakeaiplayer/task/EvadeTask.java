...
public final class EvadeTask extends AbstractTask {
    ...
    protected void onTick(AIPlayerEntity bot) {
        ...
        if (bot.blockPosition().distSqr(escapeGoal) <= 6.25D) {
    ...
    private BlockPos chooseGoal(AIPlayerEntity bot) {
        ...
        if (threat.entity() != null) {
            away = bot.position().subtract(threat.entity().position());
        ...
        away = away.normalize().scale(20.0D);
        BlockPos base = BlockPos.containing(bot.position().add(away));
