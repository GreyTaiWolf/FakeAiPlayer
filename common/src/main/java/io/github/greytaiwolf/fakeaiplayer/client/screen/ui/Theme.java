...
public final class Theme {
    ...
    public static void divider(GuiGraphics context, Font renderer, int x, int y, int w, String labelKey) {
        String label = tr(labelKey);
        int labelW = renderer.width(label);
        int lineY = y + 5;
        context.hLine(x, x + Math.max(0, (w - labelW) / 2 - 4), lineY, BORDER);
        context.drawString(renderer, label, x + Math.max(0, (w - labelW) / 2), y, TEXT_DIM);
        context.hLine(x + Math.min(w - 1, (w + labelW) / 2 + 4), x + w - 1, lineY, BORDER);
    }
