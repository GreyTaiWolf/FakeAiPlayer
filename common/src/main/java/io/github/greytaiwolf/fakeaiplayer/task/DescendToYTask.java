...
public final class DescendToYTask extends AbstractTask {
    ...
    protected void onStart(AIPlayerEntity bot) {
        ...
        io.github.greytaiwolf.fakeaiplayer.action.EquipAction.equipBestArmor(bot, "descend_start");
    }
    ...
    protected void onTick(AIPlayerEntity bot) {
        ...
        if (bot.blockPosition().getY() <= targetY) {
            miner.cancel(bot);
            complete();
            return;
        }
    ...
    private boolean tryLateralDetour(AIPlayerEntity bot, ServerLevel world, BlockPos feet) {
        ...
        for (int dy = 0; dy <= 1; dy++) {
            ...
            for (Direction dir : HORIZONTAL) {
                ...
                if (isLava(world, side) || isLava(world, side.above()) || isLava(world, side.below())) {
                    continue; // 别往岩浆方向横移
                }
                BlockPos solid = firstSolid(world, side, side.above());
                if (solid != null) {
                ...
                miner.cancel(bot);
                if (!io.github.greytaiwolf.fakeaiplayer.mode.FakePlayerMotion.stepTo(bot, side, "descend_lava_detour")) {
                    continue;
                }
                lateralDetours++;
                BotLog.action(bot, "descend_lava_detour", "dir", dir.getSerializedName(), "at_y", side.getY(), "up", dy);
                return true;
            }
        }
        return false;
    ...
    private boolean rotateStair(ServerLevel world, BlockPos feet) {
        for (int i = 0; i < HORIZONTAL.length; i++) {
            stairDirIndex = (stairDirIndex + 1) % HORIZONTAL.length;
            BlockPos ahead = feet.relative(HORIZONTAL[stairDirIndex]);
    ...
    private static boolean adjacentLava(ServerLevel world, BlockPos pos) {
        for (Direction d : Direction.values()) {
            if (isLava(world, pos.relative(d))) {
                return true;
            }
        }
        return false;
    ...
    private void maybePlaceTorch(AIPlayerEntity bot, ServerLevel world, BlockPos feet) {
        ...
        if (torchSlot.isEmpty()) {
            ...
            var coal = InventoryAction.findItem(bot, net.minecraft.world.item.Items.COAL);
            var stick = InventoryAction.findItem(bot, net.minecraft.world.item.Items.STICK);
            if (coal.isPresent() && stick.isPresent()
                    && InventoryAction.removeItems(bot, net.minecraft.world.item.Items.COAL, 1)
                    && InventoryAction.removeItems(bot, net.minecraft.world.item.Items.STICK, 1)) {
                InventoryAction.giveItem(bot, new net.minecraft.world.item.ItemStack(net.minecraft.world.item.Items.TORCH, 4));
                torchSlot = InventoryAction.findItem(bot, net.minecraft.world.item.Items.TORCH);
            }
        }
        if (torchSlot.isPresent()) {
            int held = bot.getInventory().selected;
