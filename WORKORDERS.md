# FakeAiPlayer 历史工单队列 · RL-1 … RL-20

> 这是从上游 AIBot 开发阶段保留下来的历史工程记录，其中的旧类名、Yarn/Fabric 单加载器命令和 `/aibot` 示例不代表当前构建方式。当前项目身份、双加载器结构、命令和验证步骤以根目录 `README.md` 为准；不要直接把本文件当作新的自动执行说明。
