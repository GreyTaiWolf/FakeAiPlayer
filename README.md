...
> [!IMPORTANT]
> 当前版本仍是 Alpha 开发版，不是生产兼容性认证版。仓库中已有的双加载器、单元测试、Fabric GameTest 与 NeoForge 专服记录属于各自固定基线。模块住宅实现先在 Temurin Java 21.0.11 下完成 Java 21 源码语法解析、纯生成器编译/12,800 组方案烟雾与静态门禁；随后同步提交 `dfb0ab8` 的 GitHub Actions 全矩阵通过 `:common:test`、10 项 Fabric GameTest、Fabric/NeoForge 生产构建、产物检查、两 JVM 持久化与 strict-survival runtime/evidence。该功能仍标记为 `UNVERIFIED`，因为双加载器客户端投影、真实模型调用、完整生存备料、复杂地形和长时间施工尚未完成实机验收。

## English summary

FakeAiPlayer is an experimental Minecraft 1.21.3 mod that combines a real server-side player entity, an OpenAI-compatible planner, and deterministic task state machines. This fork uses `common` code with Fabric and NeoForge adapters. Existing release claims remain baseline-specific. The modular-building implementation passed the full GitHub Actions matrix on synchronization commit `dfb0ab8`, including common tests, all 10 Fabric GameTests, both production builds, artifact checks, persistence, and runtime evidence. It remains unverified for real Fabric/NeoForge client rendering, full survival construction, complex terrain, and long-running play evidence.
...
| Fabric 接入 | 当前构建与 24/24 GameTest 已通过 | 包含初始化、生命周期、命令、聊天、断线、完整 payload、原版式 Bot Menu 和客户端事件接入；本地生产构建与全部 24 项无头 GameTest 已通过。完整客户端交互仍待人工验证。 |
| NeoForge 接入 | 当前生产构建已通过 | 包含 `@Mod` 入口、事件总线、完整 payload、Menu 注册、服务端/客户端传输和按键；本地生产构建通过，既有专用服烟雾覆盖启动、Bot 生成/列出/移除及正常停止。完整客户端交互仍待人工验证。 |
...
| 模块住宅与投影 | 双加载器接线，未验证 | 确定性住宅含地基、地板、梁柱、门窗、门廊、入口台阶、永久室内楼梯、可步行阁楼和人字顶；Fabric 与 NeoForge 均已接入服务端权威分块投影、彩色线框与人工确认。双加载器客户端实机和真实生存施工证据尚缺。 |
...
| 单元测试 | 仅保留已知 Windows 环境失败 | 当前 `:common:test` 共 189 项：187 通过、1 跳过，唯一失败仍是 Windows 缺少创建符号链接权限；本轮没有新增失败。历史固定基线不应与当前发布证据混为一谈。 |
| Fabric GameTest | 当前 24/24 通过 | 独立 `gametest` source set 已接线；除既有感知与生成测试外，新增覆盖闲置暂停/恢复、干燥有界漫游、观察主人、动态墙重规划、玩家碰撞箱、首次入水上岸、缓存复验、背包权限、原版 Shift 事务、组件守恒、绑定诅咒、距离关闭、危险接管和暂停所有权。测试代码不进入发布 JAR。 |
...
当前工作树增加了一条受约束的“起草 → 预览 → 人工确认”路径。AI 不输出任意方块坐标；它只选择风格、尺寸和 seed，`modular-house-3` 确定性生成器再组合地基、地板、梁柱、墙体、窗户、双格门、门廊、入口台阶、前后永久露台、永久室内楼梯、阁楼、无方向檐口、人字坡面和屋脊。内置风格为 `oak_cottage` 与 `spruce_lodge`。
...
| 模块建筑投影 | 尚未注册新 payload/renderer | Begin/Chunk/Commit/Ready/Confirm/Cancel 与世界线框已进入源码，仍未完成真实客户端验收 |
...
| `fakeaiplayer:building_preview_begin/chunk/commit/clear` | S2C（Fabric + NeoForge） | 有界 palette/分块传输、原子发布和清理服务端建筑投影。 |
| `fakeaiplayer:building_preview_ready/confirm/cancel` | C2S（Fabric + NeoForge） | 客户端完整接收回执，以及玩家显式确认/取消；确认不上传方块表。 |

面板打开时订阅目标，关闭时取消订阅。服务端每 10 tick 最多向有效订阅者推一次展示快照；快照中的物品数据不再具有写入权限。客户端没有相应 channel 时，聊天/快捷动作的一部分可以回退到普通聊天或 `/fakeaiplayer` 命令；背包按钮使用打开 Menu 的请求，后续物品事务走原版容器同步，设置和传送仍使用各自的自定义 payload。
...
# 启动隔离的 Fabric 无头 GameTest（当前共 24 项）
...
以下结果来自不同阶段，不能把历史基线自动合并为当前证明。当前工作树使用 JDK 21.0.11、Minecraft 1.21.3、Fabric Loader 0.18.4 / Fabric API 0.114.1+1.21.3 和 NeoForge 21.3.96；历史结果与本轮结果分开列出：
...
| 当前共享单元测试 | `:common:test` 共 189 项：187 通过、1 跳过；唯一失败为已知 Windows 无创建符号链接权限基线，本轮没有新增失败。 |
| 当前工作树编译与生产构建 | `:common:compileJava :fabric:compileJava :neoforge:compileJava`、`:fabric:build` 与 `:neoforge:build` 均通过。 |
| 当前 Fabric GameTest | 无头服务器运行 24/24 通过并正常停止；覆盖闲置控制、寻路运行时复验、安全接管和原版式背包事务。 |
...
`common/src/test` 保存纯 Java/JUnit 边界测试，包括授权、配置/模式、Goal 结果、任务取消、决策 session、执行栈和原子持久化。`fabric/src/gametest` 当前保存 24 个世界支持的 GameTest 场景。

当前分支已经重新接好 GameTest source set 和 `:fabric:runGameTest`，仓库中的 `scripts/*.sh`、`docs/*.md` 与 GitHub Actions 也已迁移到模块级任务和 `fabric/build` 运行目录。现有交互式 evidence harness 仍由 Fabric testmod 提供；NeoForge 当前自动门禁只有编译与生产构建，专用服生命周期烟雾属于此前基线，两者不能被解释为相同覆盖率。
...
- 新模块住宅目前是单层矩形主体、永久室内直梯、可步行阁楼、两套 vanilla 风格和人字顶；没有场地调查、完整房间布局、阳台、栏杆、复杂屋顶或通用模组 blockstate 适配。
...
- Fabric GameTest 当前 24/24 通过，但受控夹具不能替代真实自然地形、完整 Bot 生存施工、长时间任务回归或两个加载器的人工客户端交互验收。
