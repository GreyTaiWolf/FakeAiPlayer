...
| Fabric 接入 | 历史构建与 24/24 GameTest 已通过；本轮待验证 | 包含初始化、生命周期、命令、聊天、断线、完整 payload、原版式 Bot Menu 和客户端事件接入。历史 24 项无头 GameTest 已通过；本轮另有 11 个共享 P0/P1 场景接线，当前 Fabric 注册总数 35，尚未在此工作树运行。完整客户端交互仍待人工验证。 |
| NeoForge 接入 | 历史生产构建已通过；共享 GameTest 已接线待验证 | 包含 `@Mod` 入口、事件总线、完整 payload、Menu 注册、服务端/客户端传输和按键；既有专用服烟雾覆盖启动、Bot 生成/列出/移除及正常停止。本轮 11 个共享 P0/P1 GameTest、正式 `gameTestServer` 运行任务和 CI 日志归档已接线，但尚未在此工作树运行。完整客户端交互仍待人工验证。 |
...
| 单元测试 | 当前源码清单，新增项待运行 | `common/src/test` 当前包含 74 个测试类、274 个 `@Test`；本轮新增路径关键节点、目标距离进展与 A/B 振荡覆盖。历史与本轮通过结论必须分别绑定对应 Java 21 CI。 |
| Fabric GameTest | 历史 24/24 通过；当前源码 35 项 | 历史 24 项无头 GameTest 已通过；本轮新增 11 个 loader-neutral P0/P1 场景，Fabric wrapper 已注册但尚未运行。测试代码与共享结构资源使用隔离 source set，不进入发布 JAR。 |
| NeoForge GameTest | 当前源码 11 项，已接线待运行 | 11 个 P0/P1 场景与 Fabric 共用测试体和 batch id；NeoForge wrapper、`gameTestServer`、命名空间、隔离运行目录以及 CI 日志/crash report 归档已接线。本工作树尚无 Java 21 运行通过记录。 |
...
# 启动隔离的 Fabric 无头 GameTest（当前源码注册 35 项：历史 24 + 本轮共享 11）
./gradlew :fabric:runGameTest

# 启动隔离的 NeoForge GameTest server（当前共享 11 项）
...
以下结果来自不同阶段，不能把历史基线自动合并为当前证明。相关 Java 21 历史记录使用 JDK 21.0.11；项目当前目标仍是 Minecraft 1.21.3、Fabric Loader 0.18.4 / Fabric API 0.114.1+1.21.3 和 NeoForge 21.3.96。本轮 P0/P1 在此工作树尚未取得 Java 21 Gradle 运行结果，历史结果与本轮结果分开列出：
...
| 当前共享单元测试 | 源码清单为 74 个测试类、274 个 `@Test`；本轮新增路径关键节点、目标距离进展与 A/B 振荡回归，通过结论待本提交 Java 21 CI。历史 Windows 符号链接权限问题不能替代当前结果。 |
| 此前同步工作树编译与生产构建 | `:common:compileJava :fabric:compileJava :neoforge:compileJava`、`:fabric:build` 与 `:neoforge:build` 均通过；本轮 P0/P1 新代码尚未在此工作树用 Java 21 重跑。 |
| 历史 Fabric GameTest 基线 | 无头服务器运行 24/24 通过并正常停止；覆盖闲置控制、寻路运行时复验、安全接管和原版式背包事务。 |
| 本轮共享 P0/P1 GameTest | 共享测试体精确为 11 个场景；Fabric 当前源码注册总数 35，NeoForge 注册 11。本轮两端 wrapper、NeoForge GameTest server 和 CI 诊断归档均已接线，但尚未在此工作树运行。 |
...
`common/src/test` 保存纯 Java/JUnit 边界测试，包括授权、配置/模式、Goal 结果、任务取消、决策 session、执行栈、原子持久化和本轮路径进展回归。当前源码包含 74 个测试类、274 个 `@Test`。历史 Fabric-only 世界回归有 24 项；`common/src/gametest` 另保存 11 个本轮共享 P0/P1 场景，由 Fabric 与 NeoForge wrapper 注册，因此当前 Fabric 源码注册总数是 35，NeoForge 是 11。

当前分支已经接好 Fabric 的 `:fabric:runGameTest` 与 NeoForge 的 `:neoforge:runGameTestServer`；CI 和静态门禁要求执行两端任务，并在失败时保留两端服务端日志与 crash report。NeoForge 1.21.3 的结果以 Gradle 退出码和完整服务端日志为准，不假定它生成 Fabric 风格的 JUnit XML。现有交互式 evidence harness 仍只由 Fabric testmod 提供；本轮 11 个共享场景尚未在本工作树运行，不能解释为 NeoForge 已具有历史 Fabric 24 项的全部覆盖率。
...
- 历史 Fabric GameTest 基线为 24/24 通过；本轮另有 11 个共享 P0/P1 场景已同时注册到 Fabric 与 NeoForge、但尚未运行。受控夹具不能替代真实自然地形、完整 Bot 生存施工、长时间任务回归或两个加载器的人工客户端交互验收。
...
- [ ] 为 NeoForge 建立等价世界测试或专门 harness；本轮 11 个共享 P0/P1 GameTest 与 `gameTestServer` 已完成源码接线，仍待 Java 21 运行与 evidence；
