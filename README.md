...
| 单元测试 | 当前源码清单 | `common/src/test` 当前包含 59 个测试类、228 个 `@Test`，新增覆盖凭据分阶段替换、客户端原子存储、secret 脱敏、连接会话、聊天受众/限流与保守社交触发。通过结论以该提交的 Java 21 CI 为准。 |
...
| 当前共享单元测试 | 源码清单为 59 个测试类、228 个 `@Test`；本轮新增凭据/聊天回归，通过结论待本提交 Java 21 CI。历史 Windows 符号链接权限问题不能替代当前结果。 |
