...
| Fabric 接入 | 当前源码注册 55 项；P3 待 CI | 包含初始化、生命周期、命令、聊天、断线、完整 payload、原版式 Bot Menu 和客户端事件接入。代码提交 `2450e13` 的 CI #129 已通过历史 24 项、共享 P0/P1 11 项及共享 P2 14 项；新增 6 个 P3 场景等待当前集成 CI，完整客户端交互仍待人工验证。 |
| NeoForge 接入 | 当前源码注册 31 项；P3 待 CI | 包含 `@Mod` 入口、事件总线、完整 payload、Menu 注册、服务端/客户端传输和按键。代码提交 `2450e13` 的 CI #129 已通过共享 P0/P1 11 项与共享 P2 14 项；新增 6 个 P3 场景、正式 `gameTestServer`、生产构建及持久化恢复等待当前集成 CI，完整客户端交互仍待人工验证。 |
...
| 单元测试 | 当前源码 95 类、479 个 `@Test`；P3 待 CI | P2 的 79/312 已在代码提交 `2450e13` 的 CI #129 通过；P3 新增 Mission/Skill、组合计划、运行门禁、仲裁与 V3 恢复回归，等待当前集成提交的 Java 21 CI。 |
| Fabric GameTest | 当前源码注册 55 项；P3 待 CI | 历史 24 项、共享 P0/P1 11 项及 14 个 P2 场景已通过；新增 6 个 loader-neutral P3 场景，测试代码与共享结构资源使用隔离 source set，不进入发布 JAR。 |
| NeoForge GameTest | 当前源码注册 31 项；P3 待 CI | 共享 P0/P1 11 项及 14 个 P2 场景已通过；新增与 Fabric 使用相同测试体、batch 和阈值的 6 个 P3 场景。 |
...
| 当前共享单元测试 | 源码清单为 95 个测试类、479 个 `@Test`；P2 的 79/312 已在代码提交 `2450e13` 的 Java 21 CI #129 通过，P3 增量等待当前集成提交 CI。 |
...
| 当前共享 P3 GameTest | 增量精确为 6 个场景，当前总注册 Fabric 55、NeoForge 31；两端使用相同共享测试体、batch 和阈值，等待当前集成提交 CI。 |
...
`common/src/test` 保存纯 Java/JUnit 边界测试，包括授权、配置/模式、Goal 结果、任务取消、决策 session、执行栈、原子持久化、导航以及 Mission/Skill 契约回归。当前源码包含 95 个测试类、479 个 `@Test`。历史 Fabric-only 世界回归有 24 项；`common/src/gametest` 保存 11 个共享 P0/P1 场景、14 个共享 P2 场景和 6 个共享 P3 场景，因此当前 Fabric 源码注册总数是 55，NeoForge 是 31。

当前分支已经接好 Fabric 的 `:fabric:runGameTest` 与 NeoForge 的 `:neoforge:runGameTestServer`；CI 和静态门禁要求执行两端任务，并在失败时保留两端服务端日志与 crash report。P0/P1 的 11 个共享场景已由 CI #106 在两端通过；P2 的 14 个共享场景已由代码提交 `2450e13` 的 CI #129 在两端通过；P3 的 6 个共享场景等待当前集成提交 CI。NeoForge 1.21.3 的结果以 Gradle 退出码和完整服务端日志为准，不假定它生成 Fabric 风格的 JUnit XML。剩余 24 个 Fabric-only 世界场景和交互式 evidence harness 尚无完整 NeoForge 等价实现。
