...
| `fakeaiplayer:building_preview_begin/chunk/commit/clear` | S2C，当前仅 NeoForge | 有界 palette/分块传输、原子发布和清理服务端建筑投影。 |
| `fakeaiplayer:building_preview_ready/confirm/cancel` | C2S，当前仅 NeoForge | 客户端完整接收回执，以及玩家显式确认/取消；确认不上传方块表。 |
...
- 建筑投影当前只接通 NeoForge，且仍缺真实客户端/生存 GameTest；Fabric 客户端没有这套投影 payload 和 renderer。
