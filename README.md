...
> [!IMPORTANT]
> 当前版本仍是 Alpha 开发版，不是生产兼容性认证版。仓库中的运行结论都绑定具体提交。Bot 智能重构 P0/P1 在提交 [`9155c9c`](https://github.com/GreyTaiWolf/FakeAiPlayer/commit/9155c9ca18cb1c8021bf50d453bbafe84f2c1489) 的 [CI #106](https://github.com/GreyTaiWolf/FakeAiPlayer/actions/runs/29862817627) 中通过；P2 导航在代码提交 [`2450e13`](https://github.com/GreyTaiWolf/FakeAiPlayer/commit/2450e1352f27e3768e0f2d0ceeed98c7d7a71c4a) 的 [CI #129](https://github.com/GreyTaiWolf/FakeAiPlayer/actions/runs/29879516182) 中通过 Java 21、当时 79 个 JUnit 类/312 个 `@Test`、Fabric 49/49、NeoForge 25/25、双端生产构建与产物检查、两进程持久化及 strict-survival runtime/evidence。P2/P3 已通过 [PR #14](https://github.com/GreyTaiWolf/FakeAiPlayer/pull/14) 合入 `main`；主线提交 [`6437764`](https://github.com/GreyTaiWolf/FakeAiPlayer/commit/643776427eef46336716daaaae656e2f1c01f0d2) 的 [CI #152](https://github.com/GreyTaiWolf/FakeAiPlayer/actions/runs/29993980028) 已通过 Java 21、95 个 JUnit 类/480 个 `@Test`、Fabric 55/55、NeoForge 31/31、双端生产构建与产物检查、双 JVM V3 恢复及 strict-survival runtime/evidence。上述结果尚未由 `reports/baselines/index.tsv` 显式 pin，因此不称为 `VERIFIED` 发布基线。半透明真实 BlockState 模型投影仍标记为 `UNVERIFIED`，因为双加载器客户端实机、完整生存备料、复杂地形和长时间施工尚未完成验收。

## English summary

FakeAiPlayer is an experimental Minecraft 1.21.3 mod that combines a real server-side player entity, an OpenAI-compatible planner, and deterministic task state machines. This fork uses `common` code with Fabric and NeoForge adapters. Existing release claims remain commit-specific. Bot-intelligence P0/P1 passed on commit `9155c9c` in CI #106. P2 navigation passed Java 21, 79 JUnit classes/312 `@Test` methods, Fabric 49/49, NeoForge 25/25, both production builds and artifact checks, two-process persistence, and strict-survival runtime/evidence on code commit `2450e13` in CI #129. P2/P3 is now merged into `main` through PR #14. Main commit `6437764` passed Java 21, 95 JUnit classes/480 tests, Fabric 55/55, NeoForge 31/31, both production builds and artifact checks, two-JVM V3 persistence, and strict-survival runtime/evidence in CI #152. No result is pinned in `reports/baselines/index.tsv`, so it is not a `VERIFIED` release baseline.
...
| Fabric 接入 | 当前源码注册 55 项；P3 已通过主线 CI | 包含初始化、生命周期、命令、聊天、断线、完整 payload、原版式 Bot Menu 和客户端事件接入。主线提交 `6437764` 的 CI #152 已通过全部 55 项、生产构建与产物检查；完整客户端交互仍待人工验证。 |
| NeoForge 接入 | 当前源码注册 31 项；P3 已通过主线 CI | 包含 `@Mod` 入口、事件总线、完整 payload、Menu 注册、服务端/客户端传输和按键。主线提交 `6437764` 的 CI #152 已通过全部 31 项、正式 `gameTestServer`、生产构建、产物检查及双 JVM 持久化恢复；完整客户端交互仍待人工验证。 |
...
| 单元测试 | 当前源码 95 类、480 个 `@Test`；P3 已通过主线 CI | P2/P3 主线提交 `6437764` 的 CI #152 已通过 Java 21 全矩阵，包括 Mission/Skill、组合计划、运行门禁、仲裁与 V3 恢复回归。 |
| Fabric GameTest | 当前源码注册 55 项；P3 已通过主线 CI | 主线提交 `6437764` 的 CI #152 已通过历史 24 项、共享 P0/P1 11 项、P2 14 项及 P3 6 项；测试代码与共享结构资源使用隔离 source set，不进入发布 JAR。 |
| NeoForge GameTest | 当前源码注册 31 项；P3 已通过主线 CI | 主线提交 `6437764` 的 CI #152 已通过共享 P0/P1 11 项、P2 14 项及与 Fabric 使用相同测试体、batch 和阈值的 P3 6 项。 |
...
# 启动隔离的 Fabric 无头 GameTest（当前源码注册 55 项：历史 24 + P0/P1 11 + P2 14 + P3 6）
./gradlew :fabric:runGameTest

# 启动隔离的 NeoForge GameTest server（当前共享 31 项：P0/P1 11 + P2 14 + P3 6）
...
| 当前共享单元测试 | 源码清单为 95 个测试类、480 个 `@Test`；P2/P3 已在主线提交 `6437764` 的 Java 21 CI #152 全部通过。 |
...
| 当前共享 P3 GameTest | 增量精确为 6 个场景，当前总注册 Fabric 55、NeoForge 31；两端使用相同共享测试体、batch 和阈值，已在主线提交 `6437764` 的 CI #152 通过。 |
...
`common/src/test` 保存纯 Java/JUnit 边界测试，包括授权、配置/模式、Goal 结果、任务取消、决策 session、执行栈、原子持久化、导航以及 Mission/Skill 契约回归。当前源码包含 95 个测试类、480 个 `@Test`。历史 Fabric-only 世界回归有 24 项；`common/src/gametest` 保存 11 个共享 P0/P1 场景、14 个共享 P2 场景和 6 个共享 P3 场景，因此当前 Fabric 源码注册总数是 55，NeoForge 是 31。

当前分支已经接好 Fabric 的 `:fabric:runGameTest` 与 NeoForge 的 `:neoforge:runGameTestServer`；CI 和静态门禁要求执行两端任务，并在失败时保留两端服务端日志与 crash report。P0/P1 的 11 个共享场景已由 CI #106 在两端通过；P2 的 14 个共享场景已由代码提交 `2450e13` 的 CI #129 在两端通过；P3 的 6 个共享场景已由主线提交 `6437764` 的 CI #152 在两端通过。NeoForge 1.21.3 的结果以 Gradle 退出码和完整服务端日志为准，不假定它生成 Fabric 风格的 JUnit XML。剩余 24 个 Fabric-only 世界场景和交互式 evidence harness 尚无完整 NeoForge 等价实现。
...
- 提交 `9155c9c` 的 CI #106 已运行 Fabric 35/35 与 NeoForge 11/11，其中包含 11 个双加载器共享 P0/P1 场景；代码提交 `2450e13` 的 CI #129 已进一步运行 Fabric 49/49 与 NeoForge 25/25，其中 P2 增量是两端同源的 14 个场景。当前源码另含 6 个双端 P3 场景，总注册数为 Fabric 55、NeoForge 31，已在主线提交 `6437764` 的 CI #152 通过。剩余 24 个 Fabric-only 世界场景和交互式 harness 尚无完整 NeoForge 等价实现。受控夹具仍不能替代真实自然地形、完整 Bot 生存施工、长时间任务回归或两个加载器的人工客户端交互验收。
...
- [ ] 6 个共享 P3 场景在当前主线集成提交由 Fabric/NeoForge 同源运行通过；
