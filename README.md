...
| Fabric 接入 | 已构建并通过服务端 GameTest | 包含初始化、生命周期、命令、聊天、断线、payload 和客户端事件接入；生产 JAR 已生成，9/9 无头 GameTest 通过。客户端仍待人工验证。 |
...
| 确定性玩法 | 已保留 | 源码包含 64 个工具定义、9 类类型化 Goal，以及采集、挖矿、合成、冶炼、建造、战斗、农牧等任务状态机。 |
...
| 单元测试 | 88/88 通过 | `common/src/test` 的 25 个 JUnit 测试类已在 Java 21、Minecraft 1.21.3 / NeoForge 测试运行时执行。 |
| Fabric GameTest | 9/9 通过 | 独立 `gametest` source set 已接线；无头服务端覆盖基础 smoke case 与语义注视的方块、实体、遮挡、掉落物、漂移和容器读取。测试代码不进入发布 JAR。 |
...
- 当前 `ToolRegistry` 注册 64 个工具，覆盖回复、语义注视、观察/移动、合成、采集、挖矿、目标、容器、战斗、生存、记忆、协作和控制。
...
| 当前共享单元测试 | 25 个测试类、88/88 通过。 |
| 当前 Fabric GameTest | 无头服务器正常启动，9/9 通过并正常停止。 |
...
- Fabric GameTest 已接线并在当前工作树中 9/9 通过；它们覆盖关键确定性与语义注视场景，但仍不能替代完整长时间任务回归。
