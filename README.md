...
| Fabric 接入 | 当前构建与 10/10 GameTest 已通过 CI | 包含初始化、生命周期、命令、聊天、断线、完整 payload 和客户端事件接入；同步提交已通过生产构建与全部 10 项无头 GameTest。客户端仍待人工验证。 |
| NeoForge 接入 | 当前生产构建已通过 CI | 包含 `@Mod` 入口、事件总线、完整 payload、服务端/客户端传输和按键；同步提交已通过生产构建，既有专用服烟雾覆盖启动、Bot 生成/列出/移除及正常停止。客户端仍待人工验证。 |
...
| 单元测试 | 当前完整套件已通过 CI | 当前源码共有 42 个 `*Test.java`、175 个 `@Test`；同步提交的 `:common:test` 已通过。历史 25 类/88 测试仍只是旧的固定基线，不应与当前发布证据混为一谈。 |
| Fabric GameTest | 当前 10/10 已通过 CI | 独立 `gametest` source set 已接线；无头服务端覆盖基础 smoke case、语义注视的方块/实体/遮挡/掉落物/漂移/容器读取，以及模块住宅生成到真实注册表 BlockState、依赖序列和小范围世界支撑合同。测试代码不进入发布 JAR；这仍不是完整 Bot 施工 E2E。 |
...
# 启动隔离的 Fabric 无头 GameTest（当前共 10 项）
...
以下结果来自不同阶段，不能合并成“当前建筑工作树全部通过”。此前基线使用 JDK 21.0.11、Minecraft 1.21.3、Fabric Loader 0.18.4 / Fabric API 0.114.1+1.21.3 和 NeoForge 21.3.96；当前建筑工作树的新增诊断单独列出：
...
| 当前共享单元测试 | 历史基线为 25 个测试类、88/88 通过；当前源码已增至 42 类/175 个 `@Test`，完整 Gradle/JUnit 套件仍待 GitHub Actions。 |
| 当前建筑工作树完整 Gradle | 已用 Java 21 + Gradle 8.11 发起 `:common:test :neoforge:build`；当前环境的 JVM 在解析外部 Maven 上的 `fabric-loom` 时失败，尚未进入项目源码编译，不能标记通过。 |
...
`common/src/test` 保存纯 Java/JUnit 边界测试，包括授权、配置/模式、Goal 结果、任务取消、决策 session、执行栈和原子持久化。`fabric/src/gametest` 保存 10 个世界支持的 GameTest 场景。
...
- Fabric GameTest 已接线，9/9 是此前固定基线；当前第 10 项只验证模块住宅的编译、真实 BlockState、依赖序列和小范围支撑，仍待 GitHub Actions，且不能替代完整 Bot 生存施工或长时间任务回归。
