...
| 层级 | 入口 | 当前规模 | 作用 |
|---|---|---:|---|
| JUnit | `./gradlew :common:test` | 当前源码 74 个测试类、274 个 `@Test` | 验证纯 Java 策略、权限、Goal predicate/result、语义注视、暂停所有权、寻路、序列化、凭据/聊天边界，以及本轮路径关键节点、目标距离进展和 A/B 振荡。新增项尚未在此工作树用 Java 21 运行。历史固定基线不能代替当前结果。 |
| Fabric GameTest | `./gradlew :fabric:runGameTest` | 当前源码注册 35 项 | 历史 24/24 已通过；本轮另注册 11 个 loader-neutral P0/P1 场景，尚未在此工作树运行。Fabric 结果另可写 JUnit XML，但不能把历史结果当作本轮结果。 |
| NeoForge GameTest | `./gradlew :neoforge:runGameTestServer` | 当前源码注册 11 项 | 与 Fabric 共用同一测试体和 batch id；wrapper、正式 GameTest server、命名空间与隔离运行目录已接线。本轮结果以 Gradle 退出码、完整服务端日志和 crash report 为准，不假定生成 JUnit XML。 |
...
当前源码清单是 74 个 JUnit 类、274 个 `@Test`、35 个 Fabric GameTest 注册和 11 个 NeoForge GameTest 注册。PR #8 的历史 Java 21 CI 已确认当时的完整 JUnit、24/24 Fabric GameTest、双加载器生产构建与两 JVM persistence probe 通过；这仍是历史固定基线。本轮新增的 11 个共享 P0/P1 场景已同时接入两端，但在此工作树上尚无 Java 21 运行结果，不能写成 35/35 或 11/11 已通过。受控测试仍不替代真实 Fabric/NeoForge 客户端、真实提供商连接、完整施工或显式 pin 的发布基线。
...
所有新增场景仍待 Java 21 双加载器运行。共享 wrapper 保持默认单次尝试；涉及单调账本的场景若
未来启用原位 retry，必须同时隔离坐标或重建账本/世界。
