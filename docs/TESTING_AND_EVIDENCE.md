...
| 层级 | 入口 | 当前规模 | 作用 |
|---|---|---:|---|
| JUnit | `./gradlew :common:test` | 25 个测试类、88 个测试 | 验证纯 Java 策略、权限、Goal predicate/result、语义注视、序列化和原子存储边界。 |
| Fabric GameTest | `./gradlew :fabric:runGameTest` | 9 个测试 | 在真实 Minecraft world context 中验证确定性 smoke case 与语义注视射线、遮挡、漂移和容器读取。 |
...
当前源码测试清单是 25 个 JUnit 类、88 个测试和 9 个 GameTest。当前本地诊断中，strict/operator 的 capability + runtime-control suite 均为 `7/7 PASS`；两 JVM persistence probe 精确恢复 checkpoint map，并在 resume 后得到原 Mission 的 `COMPLETED 4/4`。这些数字描述本次工作树的验证，不替代 clean-commit evidence gate。
