...
| 层级 | 入口 | 当前规模 | 作用 |
|---|---|---:|---|
| JUnit | `./gradlew :common:test` | 19 个测试类、68 个测试 | 验证纯 Java 策略、权限、Goal predicate/result、序列化和原子存储边界。 |
| Fabric GameTest | `./gradlew :fabric:runGameTest` | 3 个测试 | 在真实 Minecraft world context 中验证确定性 smoke case。 |
