...
| 层级 | 入口 | 当前规模 | 作用 |
|---|---|---:|---|
| JUnit | `./gradlew :common:test` | 当前源码 84 个测试类、338 个 `@Test` | 验证纯 Java 策略、权限、Goal predicate/result、语义注视、暂停所有权、寻路、序列化、凭据/聊天边界，以及导航与 Mission/Skill 契约；P2 的 79 类/312 项已在代码提交 `2450e13` 的 CI #129 通过，P3 增量仍须由当前提交 CI 重新验证。 |
| Fabric GameTest | `./gradlew :fabric:runGameTest` | 当前源码注册 53 项 | P2 基线 49/49 已由 CI #129 通过；当前另有 4 个 loader-neutral P3 场景，须由当前提交 CI 重新验证。 |
| NeoForge GameTest | `./gradlew :neoforge:runGameTestServer` | 当前源码注册 29 项 | P2 基线 25/25 已由 CI #129 通过；当前与 Fabric 共用相同测试体、batch 和阈值的 4 个 P3 场景，须由当前提交 CI 重新验证。结果以 Gradle 退出码、完整服务端日志和 crash report 为准。 |
...
P3 V1 增量为 **4 个**双加载器共享场景：REFLEX 临时抢占后恢复精确 Mission frame 且中断 latch
只消费一次；护甲 Skill 必须把四件护甲与剑放入权威装备槽，并覆盖“斧头攻击更高但 Goal 明确要求剑”
及 offhand 剑交换；Task 的 pause/resume hook 抛异常时 active slot、暂停栈与 USER lock 保持事务一致；
Task tick 抛异常时结构化失败并退出 active slot，而不是打断服务器 tick。
