...
| 层级 | 入口 | 当前规模 | 作用 |
|---|---|---:|---|
| JUnit | `./gradlew :common:test` | 当前源码 47 个测试类、189 个 `@Test` | 验证纯 Java 策略、权限、Goal predicate/result、语义注视、暂停所有权、寻路策略、序列化和原子存储边界。历史 25/88 仍只是旧的固定基线。 |
...
当前源码清单是 47 个 JUnit 类、189 个 `@Test` 和 24 个 Fabric GameTest。PR #8 的首次 Java 21 CI 已确认完整 JUnit、24/24 GameTest、双加载器生产构建与两 JVM persistence probe 通过；随后 strict-survival runtime contract 暴露了闲置动作验收漂移和嵌套安全暂停恢复问题，本次修复必须以新的全绿 run 为准。受控测试仍不替代真实客户端、完整施工或显式 pin 的发布基线。
