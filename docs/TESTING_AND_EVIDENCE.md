...
| 层级 | 入口 | 当前规模 | 作用 |
|---|---|---:|---|
| JUnit | `./gradlew :common:test` | 当前源码 95 个测试类、479 个 `@Test` | 验证纯 Java 策略、权限、Goal predicate/result、语义注视、暂停所有权、寻路、序列化、凭据/聊天边界，以及导航与 Mission/Skill 契约；P2 的 79 类/312 项已在代码提交 `2450e13` 的 CI #129 通过，P3 完成增量仍须由当前提交 CI 重新验证。 |
| Fabric GameTest | `./gradlew :fabric:runGameTest` | 当前源码注册 55 项 | P2 基线 49/49 已由 CI #129 通过；当前另有 6 个 loader-neutral P3 场景，须由当前提交 CI 重新验证。 |
| NeoForge GameTest | `./gradlew :neoforge:runGameTestServer` | 当前源码注册 31 项 | P2 基线 25/25 已由 CI #129 通过；当前与 Fabric 共用相同测试体、batch 和阈值的 6 个 P3 场景，须由当前提交 CI 重新验证。结果以 Gradle 退出码、完整服务端日志和 crash report 为准。 |
...
当前源码清单是 95 个 JUnit 类、479 个 `@Test`、55 个 Fabric GameTest 注册和 31 个 NeoForge GameTest 注册。提交 [`9155c9c`](https://github.com/GreyTaiWolf/FakeAiPlayer/commit/9155c9ca18cb1c8021bf50d453bbafe84f2c1489) 的 [CI #106](https://github.com/GreyTaiWolf/FakeAiPlayer/actions/runs/29862817627) 已通过 P0/P1；代码提交 [`2450e13`](https://github.com/GreyTaiWolf/FakeAiPlayer/commit/2450e1352f27e3768e0f2d0ceeed98c7d7a71c4a) 的 [CI #129](https://github.com/GreyTaiWolf/FakeAiPlayer/actions/runs/29879516182) 已通过当时的 Java 21 JUnit、Fabric 49/49、NeoForge 25/25、Fabric/NeoForge 生产构建与产物检查、两进程持久化和 strict-survival runtime/evidence。相对该 P2 基线，P3 当前新增 16 个 JUnit 类/167 项及两端同源的 6 个世界场景；其中完整 Mission 运行时完成批次相对 P3 V1 分支新增 11 个 JUnit 类/141 项和 2 个世界场景。这些增量只有当前提交 CI 实际通过后才能记为提交绑定证据。上述旧 run 尚未由 `reports/baselines/index.tsv` 显式 pin，因此不是 `VERIFIED` 发布 baseline。受控测试仍不替代真实 Fabric/NeoForge 客户端、真实提供商连接或完整施工。
...
P3 增量为 **6 个**双加载器共享场景：跨维度 Mission 在 `Task.start` 前失败关闭，REFLEX 临时抢占
后恢复精确 Mission frame 且中断 latch 只消费一次；护甲 Skill 必须把四件护甲与剑放入权威装备槽，并覆盖“斧头攻击更高但 Goal 明确要求剑”
及 offhand 剑交换；Task 的 pause/resume hook 抛异常时 active slot、暂停栈与 USER lock 保持事务一致；
Task tick 抛异常时结构化失败并退出 active slot，而不是打断服务器 tick；Goal 来源和优先级在真实
Task 安装中保持权威；空背包 Bot 必须实际砍树、合成木镐和石镐、采集石头和铁矿并熔炼出铁锭，
同时验证 Mission id/source/priority、技能顺序、库存结果和世界变化。
Fabric 与 NeoForge 使用相同共享实现；在当前提交 CI 实际完成前不得写成已通过。
