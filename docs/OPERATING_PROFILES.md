...
FakeAiPlayer 有两种运行模式：默认的 `strict_survival` 与兼容用途的 `operator`。模式控制的是特权边界，不改变 Goal/Task 的基本架构。

## 配置

推荐在 `fakeaiplayer.json` 中显式写出 profile：
...
```bash
AIBOT_PROFILE=strict_survival ./gradlew :fabric:runServer
```

NeoForge 开发服务端使用 `./gradlew :neoforge:runServer`。`AIBOT_PROFILE` 是上游已公开的运行时兼容键，当前仍保留；测试脚本同时接受新的 `FAKEAIPLAYER_PROFILE` 别名。
