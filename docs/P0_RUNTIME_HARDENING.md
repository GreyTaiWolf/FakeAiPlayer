...
- `IntentController.replace()` 已接面板直接派工、`/fakeaiplayer task` 和 `/fakeaiplayer memory goto`；LLM 生产路径通过同批 `stop + Goal/Action` 实现等价替换；
...
- `./gradlew :common:test` 不再是 `NO-SOURCE`；
- 每个 P0 bug 至少有一个先红后绿的自动化测试；
- CI 对失败返回非零状态且保存诊断产物。

当前：生产 jar 与 sources jar 均经内容检查，不含 GameTest 类和 `/fakeaiplayer test`、`/fakeaiplayer verify`；命令驱动 harness 位于 `fabric/src/gametest`。PR CI 运行 JUnit、GameTest、build、两 JVM restart 与 strict evidence；nightly 运行 strict/operator × 多 seed；真实 LLM story 只能手动确认计费后执行。所有 workflow 在失败时上传诊断与 evidence。
